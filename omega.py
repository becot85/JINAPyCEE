...
#### trueman edits
    ...
    def plot_spectro(self,fig=3,xaxis='age',yaxis='[Fe/H]',source='all',label='',shape='-',marker='o',color='k',markevery=100,show_data=False,show_sculptor=False,show_legend=True,return_x_y=False,sub_plot=False,linewidth=3,sub=1,plot_data=False,plot_for_radial=False,fsize=[10,4.5],fontsize=14,rspace=0.6,bspace=0.15,labelsize=15,legend_fontsize=14,only_one_iso=False,solar_ab='',sfr_thresh=0.0,m_formed_thresh=1.0,solar_norm=''):
        '''
        ...
        if not plot_data:
          if plot_for_radial:
              ...
              #filtrate bad value
              #filtrate = np.isfinite(np.array(x)*np.array(y))
              #x = np.array(x)[filtrate]
              #y = np.array(y)[filtrate]
